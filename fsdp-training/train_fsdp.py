#!/usr/bin/env python3
"""
Distributed PyTorch Training using FSDP (Fully Sharded Data Parallel)
Uses Vision Transformer with high-resolution synthetic images for multi-node, multi-GPU training
FSDP shards model parameters, gradients, and optimizer states across GPUs for memory efficiency
"""
...
class VisionTransformer(nn.Module):
    """
    Large Vision Transformer model optimized for FSDP training.
    This model is computationally intensive and can be sharded across multiple GPUs.
    """
    def __init__(
        self,
        image_size=384,
...
def train_epoch(model, dataloader, optimizer, criterion, epoch, rank, local_rank,
    ...
    FSDP handles mixed precision internally via MixedPrecision policy.
    """
    model.train()
    total_loss = 0
    total_correct = 0
...
def save_checkpoint(model, optimizer, epoch, checkpoint_dir, rank):
    """
    Save checkpoint using FSDP's state_dict API.
    FSDP requires special handling to gather the full model state.
    """
    if rank == 0:
...
def main():
    parser = argparse.ArgumentParser(
        description='Distributed PyTorch Training with FSDP and Vision Transformer'
    )
    ...
    parser.add_argument('--cpu-offload', action='store_true',
                        help='Enable CPU offloading for FSDP (useful for very large models)')
    ...
    if rank == 0:
        print('='*80)
        print(f'Distributed Training with FSDP')
        print('='*80)
    ...
    # Calculate model parameters before FSDP wrapping
    ...
    sharding_strategy = sharding_strategy_map[args.sharding_strategy]

    # Configure mixed precision
    # FSDP handles mixed precision more efficiently than manual AMP
    ...
    # Configure CPU offload (optional, for very large models)
    ...
    model = FSDP(
        ...
        sync_module_states=True,  # Ensure all ranks start with same model
